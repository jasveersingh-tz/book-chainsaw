...
class MergeDecisionEngine {
  constructor(config = {}) {
    ...
    this.learningAdjustments = config.learningAdjustments || null;
  }
  ...
  aggregateIssues(results) {
    ...
    // Add issues from learned patterns FIRST
    if (this.learningAdjustments && this.learningAdjustments.additionalIssues) {
      for (const learnedIssue of this.learningAdjustments.additionalIssues) {
        issues.high.push({
          type: learnedIssue.type,
          severity: 'high',
          message: learnedIssue.suggestion,
          source: 'learned-pattern',
        });
        console.log(`   🧠 Learned Issue: ${learnedIssue.suggestion}`);
      }
    }

    // Adjust severity based on learned patterns
    if (this.learningAdjustments && this.learningAdjustments.severityAdjustments) {
      for (const adjustment of this.learningAdjustments.severityAdjustments) {
        console.log(`   🧠 Severity Adjustment: ${adjustment.pattern} - ${adjustment.reason}`);
      }
    }
  ...
  calculateScore(results) {
    ...
    // Apply learning-based score adjustments FIRST
    if (this.learningAdjustments && this.learningAdjustments.scoreAdjustments) {
      for (const adjustment of this.learningAdjustments.scoreAdjustments) {
        score += adjustment.adjustment;
        console.log(
          `   🧠 Learning: ${adjustment.reason} (${adjustment.adjustment > 0 ? '+' : ''}${adjustment.adjustment})`,
        );
      }
    }
