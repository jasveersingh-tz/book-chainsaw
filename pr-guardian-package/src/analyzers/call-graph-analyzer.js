...
class CallGraphAnalyzer {
  ...
  async detectSignatureChanges(changedFiles) {
    ...
    // Get git root once for all files
    const gitRoot = execSync('git rev-parse --show-toplevel', { encoding: 'utf-8' })
      .trim()
      .replace(/\//g, path.sep);
    ...
    for (const file of changedFiles) {
      try {
        ...
        const oldContent = execSync(`git show HEAD:${relativePath.replace(/\\/g, '/')}`, {
          encoding: 'utf-8',
        ...
        const absolutePath = path.isAbsolute(file) ? file : path.join(gitRoot, file);
        const newContent = await fs.readFile(absolutePath, 'utf-8'); // Parse both versions
